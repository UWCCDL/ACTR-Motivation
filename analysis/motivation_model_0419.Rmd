...
knitr::opts_chunk$set(echo = TRUE, cache = TRUE, warning = FALSE, error = FALSE, message = FALSE, 
                      tidy = T, cache.path = "./motivation_model_cache/20220419/", 
                      fig.path = "./motivation_model_files/figure-html/20220419/")
...
if (SKIP) {
  ...
  #load("./motivation_model_cache/20220420.RData")
...
df_model.cong <-  df_model %>% 
  ...
  group_by(epoch, condition_stimulus) %>%
  summarise(n = n(), 
...
plot.se1 <- ggplot(df_model.cong,
  ...
  geom_point(aes(col = condition_stimulus, group=1),alpha = .3, size=8) +
...
plot.se3 <- ggplot(df_model.val,
  ...
  geom_point(aes(col = condition_cue, group=1),alpha = .3, size=8) +
...
```{r eval=FALSE}
aov(response_time.mean ~ condition_stimulus * condition_cue, 
    df_model.performance_agg)  %>%
  anova_summary() %>%
  xtable() %>%
  kable(digits=5) %>%
...
df.posterror2 <- df_model %>%
  ...
  summarise(response_time.mean = mean(response_time)) %>%
  ggplot(aes(x = group, y = response_time.mean/SCALE_FACTOR, group=1, color=group)) +
  geom_point(size=8, alpha=.8) +
  scale_color_brewer(palette = "Set2") +
  stat_summary(fun.data = "mean_se", col="black", geom = "pointrange", size=1, alpha=.5, color="grat10") +
  stat_summary(fun.data = "mean_se", col="black", geom = "line", size=2, alpha=.5, color="grat10") +
...
Load Fatigue simulation data


```{r}
dfm.fatigue <- list.files("../data/", pattern = "model_output*", full.names = T) %>% map_df(~read_csv(.))
```

Next, we can compare the 

```{r}
plot.fatigue.m1 <- dfm.fatigue %>%
...
```{r}
df_trace_reduced %>%
  ggbarplot(x = "production", y =":u", fill = "production", add = "mean_ci", #position = position_dodge(0.9), 
            palette = "Set3") +
  rotate_x_text(30) + 
