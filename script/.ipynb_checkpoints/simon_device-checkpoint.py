...
def generate_stimuli(shuffle = True, n_trials=10, valid_cue_percentage=1):
    ...
    valid = congr_valid * int(n_trials * (valid_cue_percentage)) + incongr_valid * int(n_trials * (valid_cue_percentage))
    invalid = congr_invalid * int(n_trials * (1-valid_cue_percentage)) + incongr_invalid * int(n_trials * (1-valid_cue_percentage))
    lst = valid + invalid

    if shuffle:  # Randomized if needed
        random.shuffle(lst)
    
    return [SimonStimulus(shape = x[0], location = x[1], cue=x[2]) for x in lst][0:2]
...
class SimonTask:
    ...
    def modify_stimulus_feature(self):
        ...
        print("TEST: modify_stimulus_feature", feature, shape, location)
    ...
    def modify_cue_feature(self):
        id = 5 * self.index + 1
        feature = "visicon-id%d" % (id,)
        cue = self.current_trial.stimulus.cue
        print("TEST: modify_cue_feature", feature, shape, location)
        actr.modify_visicon_features([feature, "shape", "ARROW", "cue", cue.upper()])
    ...
    def update_window(self):
        ...
        if self.window is not None:
            ...
            cue = self.current_trial.stimulus.cue.upper()
            ...
            if self.phase == "fixation":
                ...
                #actr.schedule_event_relative(0.001, "stroop-modify-cue-feature", params=[])
                #time.sleep(2)
                print("TEST: in cue", item)
                actr.print_visicon()
    ...
    def next(self):
        ...
        if self.phase == "fixation":
            ...
            print("TEST: in 328 next()", self.phase)
        
        elif self.phase =="cue":
            self.phase = "stimulus"
            print("TEST: in L343 next()", self.phase)
...
def run_experiment(model="simon-model0",
    ...
    actr.add_command("stroop-modify-cue-feature", task.modify_cue_feature,
    ...
    actr.remove_command("stroop-modify-cue-feature")
