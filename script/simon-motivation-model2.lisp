...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; 
;;; Filename    :simon-motivation-model2.lisp
;;; Version     :v3.2
;;; 
...
==>
   ...
   !output! (in prepare-wm motivation value is =VAL time-onset is =TIME)
)
...
==>
   ...
   *goal>
   ...
   !output! (in retrieve-intended-response() the motivation old val is =MOT new val is =DIFF)
...
(p dont-check
   ...
   =goal>
     ...
     step       retrieve-rule
     ;time-onset   =TIME
     <= motivation   0  ;;; compete with check-pass, higher utility fires
     motivation   =MOT
==>
   *goal>
     step       check-rule
   ...
   ;!output! (in dont-check() time-onset is =TIME current time is =CURRTIME motivation is =MOT duration is =DURATION DIFF(MOT-DURATION) is =DIFF)
)
;;; Check
